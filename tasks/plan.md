...
- `src/config.rs`:
  ...
  - 環境変数 `MY_TASK_SYNC_NGROK_DOMAIN` の上書きを `resolve()` に追加
    (既存の `MY_TASK_SYNC_*` prefix 慣習に合わせる)
- `src/ngrok.rs` (新規):
...
- `MY_TASK_SYNC_NGROK_DOMAIN=foo.ngrok-free.dev cargo run` で env 経由設定が効く (config.toml より優先)
...
4. `~/.config/my-task-sync/config.toml` に `[ngrok].domain = "..."` を設定して `cargo run` → ログに "listening" + "ngrok started pid=<N>" が両方出る
...
- `README.md` / `docs/README_ja.md`:
  - Install セクションに ngrok authtoken セットアップ手順を追記:
    ...
    ```
  - Configure セクションに `[ngrok].domain` を追加 + 環境変数表に `MY_TASK_SYNC_NGROK_DOMAIN` を追加
  - Manage セクションに "公開 URL 確認" として `curl localhost:3333/api/status` を追加
- `config.example.toml`: `[ngrok]` セクションを追加 (コメントアウト形で `# domain = "..."`、未設定がデフォルトと分かる形)
